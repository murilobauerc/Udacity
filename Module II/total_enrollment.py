...
def total_enrollment(university):
    totStudents, totFees = 0,0
    for name,students,price in university:
        totStudents += students
        totFees = totFees + students * price
    return totStudents, totFees

#using -while
def total_enrollment(university):
    totStudents, totFees, i = 0,0,0
    while i < len(university):
        totStudents += university[i][1]
        totFees += university[i][1] * university[i][2]
        i += 1
    return totStudents, totFees
